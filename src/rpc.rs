...
impl RpcClient {
    ...
    /// Token accounts this owner holds that are EMPTY, with the program that
    /// owns each one.
    ///
    /// Every buy creates an associated token account, and that account holds
    /// ~0.00204 SOL of rent until it is closed. Selling the position empties it
    /// but does not close it, so the rent stays locked — measured on a live
    /// round trip, a 0.01 SOL trade returned 0.010097 SOL on the swap and still
    /// netted −0.002 because the rent never came back. At small trade sizes
    /// that is a fifth of the position, paid on every token, forever.
    ///
    /// Returns `(ata_address, owning_token_program)` so the caller can build
    /// the right close instruction — a Token-2022 account closed against the
    /// classic program simply fails.
    #[cfg(feature = "sniper")]
    pub async fn empty_token_atas(
        &self,
        owner: &str,
        mints: &[String],
    ) -> Option<Vec<(String, String)>> {
        if self.url.is_empty() || mints.is_empty() {
            return None;
        }
        use spl_associated_token_account_interface::address::
            get_associated_token_address_with_program_id;
        ...
        for chunk in mints.chunks(50) {
            ...
            for m in chunk {
                ...
                for program in [&crate::tx::TOKEN_PROGRAM, &crate::tx::TOKEN_2022_PROGRAM] {
                    addrs.push(
                        get_associated_token_address_with_program_id(&owner_pk, &mint_pk, program)
                            .to_string(),
                    );
                }
            }
            ...
            for (i, acct) in accts.iter().enumerate() {
                ...
                let amount = info
                    ...
                    .unwrap_or(1);
                // Only a genuinely empty account can be closed; a non-zero one
                // would fail the instruction and take the whole batch with it.
                if amount != 0 {
                    continue;
                }
                let Some(program) = acct.get("owner").and_then(|v| v.as_str()) else { continue };
                if let Some(addr) = addrs.get(i) {
                    out.push((addr.clone(), program.to_string()));
                }
            }
        }
        Some(out)
